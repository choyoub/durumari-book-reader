...
export default function App() {
  const [settings, setSettings] = useState<ReaderSettings>(() => ({ ...defaultSettings, ...loadJson("durumari.settings", {}) }));
  ...
  const browseSource = useCallback(async () => {
    ...
    try {
      ...
      if (selected) {
        ...
        const folderName = selected.path.split(/[/\\]/).filter(Boolean).pop() || "내 도서";
    ...
    const runSync = async () => {
      ...
      try {
        ...
        for (const source of localSources) {
          ...
          if (!isTauri && !source.handle) continue; // 웹 환경에서는 새로고침 시 핸들이 날아가므로 무시
          ...
          if (isTauri) {
            ...
            if (!(await exists(source.path!))) {
              if (!currentIsFirstRun) {
                ...
                await message(`폴더를 찾을 수 없습니다: ${source.path}\n해당 폴더와 관련된 모든 도서 기록 및 책갈피가 삭제됩니다.`, { title: "폴더 유실", kind: "warning" });
              }
              
              setSources(prev => prev.filter(s => s.id !== source.id));
                setBooks((currentBooks) => {
                  const booksToDelete = currentBooks.filter(b => b.sourceId === source.id);
                  const deletedIds = booksToDelete.map(b => b.id);
                  if (deletedIds.length > 0) {
                    ...
                    setBookmarks(prev => {
                      ...
                      localStorage.setItem("durumari.bookmarks", JSON.stringify(updated));
                      return updated;
                    });
                  }
                  return currentBooks.filter(b => b.sourceId !== source.id);
                });
  ...
  const removeSource = useCallback(async (sourceId: string) => {
    ...
    try {
      confirmed = await ask(
        "이 폴더를 목록에서 제거하시겠습니까?\n(실제 파일은 삭제되지 않습니다.)",
        { title: "폴더 제거 확인", kind: "warning", okLabel: "제거", cancelLabel: "취소" }
      );
    } catch {
  ...
  const resetSettings = useCallback(async () => {
    let confirmed = false;
    try {
      confirmed = await ask("모든 설정, 읽기 기록, 책갈피를 초기화하시겠습니까?", { title: "설정 및 데이터 초기화", kind: "warning", okLabel: "초기화", cancelLabel: "취소" });
    } catch {
  ...
  const clearAllSources = useCallback(async () => {
    let confirmed = false;
    try {
      confirmed = await ask("등록된 모든 폴더를 목록에서 해제하시겠습니까?\n(실제 파일은 삭제되지 않으며, 독서 기록은 유지됩니다.)", { title: "폴더 전체 해제", kind: "warning", okLabel: "전체 해제", cancelLabel: "취소" });
    } catch {
  ...
  const openBook = useCallback(async (selected: OpenedBook, targetProgress?: number, targetCfi?: string) => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : "책을 열지 못했습니다.");
      return;
    } finally { setLoading(false); }
    setHistory((items) => {
  ...
  useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      ...
      if (escMenu) return;
      if (["ArrowRight", "ArrowDown", "PageDown", " "].includes(event.key)) { event.preventDefault(); navRef.current?.next(); }
      if (["ArrowLeft", "ArrowUp", "PageUp"].includes(event.key)) { event.preventDefault(); navRef.current?.previous(); }
    };
    window.addEventListener("keydown", handler); return () => window.removeEventListener("keydown", handler);
  }, [book, escMenu, pageNavigator, settingsOpen]);

  useEffect(() => {
    const handleContextMenu = (event: MouseEvent) => {
      if (!book || settingsOpen || pageNavigator) return;
      event.preventDefault();
      setEscMenu((value) => !value);
    };
  ...
  const reader = useMemo(() => {
    if (!book) return null;
    if (book.kind === "epub" && book.bytes) return <Suspense fallback={<div className="loading-view">EPUB 문서를 여는 중...</div>}><EpubReader key={book.id} bytes={book.bytes} settings={settings} initialCfi={cfi} initialProgress={progress} onProgress={updateProgress} navRef={navRef} onPageInfo={setInfo} /></Suspense>;
...
function LibraryView({ tab, setTab, search, setSearch, books, sources, activeSourceId, onSelectSource, onRemoveSource, history, bookmarks, onOpen, onOpenHistory, onAdd, onSettings, activeSource, settings, setSettings }: {
  ...
  useEffect(() => setFocusedRow(null), [tab]);
  ...
  const handleHeaderClick = (tabName: Tab, column: string) => {
    const getNextDirection = (currentCol: string, currentDir: "asc" | "desc" | "none") => {
      if (currentCol !== column) return "asc";
      if (currentDir === "none") return "asc";
    ...
    if (tabName === "library") {
      setSettings((prev) => {
        ...
        return { ...prev, librarySort: { column: nextDir === "none" ? "openedAt" : column, direction: nextDir === "none" ? "desc" : nextDir as "asc" | "desc" } };
      });
    } else if (tabName === "history") {
      setSettings((prev) => {
        const current = prev.historySort || { column: "openedAt", direction: "desc" };
        const nextDir = getNextDirection(current.column, current.direction);
        return { ...prev, historySort: { column: nextDir === "none" ? "openedAt" : column, direction: nextDir === "none" ? "desc" : nextDir as "asc" | "desc" } };
      });
    } else if (tabName === "bookmarks") {
      setSettings((prev) => {
        const current = prev.bookmarksSort || { column: "createdAt", direction: "desc" };
        const nextDir = getNextDirection(current.column, current.direction);
        return { ...prev, bookmarksSort: { column: nextDir === "none" ? "createdAt" : column, direction: nextDir === "none" ? "desc" : nextDir as "asc" | "desc" } };
      });
    }
  };
            ...
            <table className="library-table">
              <thead>
                <tr>
                  ...
                  <th className="sortable" onClick={() => handleHeaderClick("library", "openedAt")}>등록일{getSortIndicator(librarySort, "openedAt")}</th>
              ...
              <tbody>
                {sortedBooks.length === 0 ? (
                  ...
                  return <tr className={`openable-row ${focusedRow === `book:${item.id}` ? "selected" : ""} ${status.className}`} key={item.id} tabIndex={0} onClick={() => setFocusedRow(`book:${item.id}`)} onDoubleClick={() => onOpen(item)} onKeyDown={(event) => event.key === "Enter" && onOpen(item)}><td className="book-name">{item.name}</td><td>{formatDate(item.openedAt)}</td><td className={status.className}>{status.label}</td></tr>;
                })}
              </tbody>
            </table>
          </div>
      ...
      {tab === "history" && (
        <div className="table-scroll-wrapper">
          <table className="history-table">
            ...
            <tbody>
              {sortedHistory.map((item) => <tr className={`openable-row ${focusedRow === `history:${item.id}` ? "selected" : ""}`} key={item.id} tabIndex={0} onClick={() => setFocusedRow(`history:${item.id}`)} onDoubleClick={() => onOpenHistory(item.id)} onKeyDown={(event) => event.key === "Enter" && onOpenHistory(item.id)}><td>💻 {getSourceName(item.id)}</td><td>{item.name}</td><td>{formatDate(item.openedAt)}</td><td className="accent-text">{Math.round(item.progress * 100)}%</td></tr>)}
            </tbody>
          </table>
        </div>
      )}
      {tab === "bookmarks" && (
        <div className="table-scroll-wrapper">
          <table className="bookmarks-table">
            ...
            <tbody>
              {sortedBookmarks.map((item) => { const rowKey = `bookmark:${item.bookId}:${item.createdAt}`; return <tr className={`openable-row ${focusedRow === rowKey ? "selected" : ""}`} key={`${item.bookId}-${item.createdAt}`} tabIndex={0} onClick={() => setFocusedRow(rowKey)} onDoubleClick={() => onOpenHistory(item.bookId, item.progress, item.cfi)} onKeyDown={(event) => event.key === "Enter" && onOpenHistory(item.bookId, item.progress, item.cfi)}><td>💻 {getSourceName(item.bookId)}</td><td>{item.bookTitle}</td><td>{formatDate(item.createdAt)}</td><td className="accent-text">p.{item.page}</td></tr>; })}
            </tbody>
          </table>
        </div>
      )}
