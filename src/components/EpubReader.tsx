...
interface Props {
  ...
  navRef: React.MutableRefObject<{ next: () => void; previous: () => void; go: (p: number) => void } | null>;
  onPageInfo: (current: number, total: number, indexing: boolean) => void;
}

export function EpubReader({ bytes, settings, initialCfi, initialProgress = 0, onProgress, navRef, onPageInfo }: Props) {
  ...
  }, [bytes, initialCfi, initialProgress, settings.fontFamily, settings.fontSize, settings.isBold, settings.lineHeight, settings.letterSpacing,
